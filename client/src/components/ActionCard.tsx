  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
      <div className="flex items-start gap-4">
        <div className={cn(
          "mt-1 p-2 rounded-lg",
          action.type === 'fund_transfer' ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400"
        )}>
          {action.type === 'fund_transfer' ? <Play size={16} /> : <Clock size={16} />}
        </div>
        <div>
      ...
      <button
        ...
        disabled={!isPending || executeMutation.isPending}
        className={cn(
      ...
      >
        {executeMutation.isPending ? "Running..." : isPending ? "Execute" : "Done"}
      </button>
    </div>
  );
}
